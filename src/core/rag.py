...
-   **HYBRID**: **[사용자 정보]**와 **[검색 문서]**를 모두 참조해야만 정확한 답변이 가능한 질문입니다. "내 카드", "내 결제일" 등 사용자 정보와 관련된 대명사나 키워드를 실제 값으로 치환해야 합니다.
-   **USER_INFO_ONLY**: **[검색 문서]** 없이, 오직 **[사용자 정보]**만으로 충분히 답변할 수 있는 질문입니다.

**3단계: 다음 행동(`action`) 결정 및 `refined_query` 생성**
-   **만약 `USER_INFO_ONLY`로 분류된다면:**
    -   `action`은 `DIRECT_ANSWER`로 설정합니다.
    -   `refined_query`에는 답변에 필요한 정보가 무엇인지 명시하는 '정보 요청 키'를 생성합니다. (예: "사용자 카드 결제일자 조회")
    - 예를 들어, 사용자가 "내 이름이 뭐야?"라고 물었을 때, 사용자 정보에 '이름: 홍길동'이 있으므로 "홍길동입니다."와 같이 바로 답변할 수 있는 경우입니다.
-   **만약 `GENERAL` 또는 `HYBRID`로 분류된다면:**
    ...
    -   `refined_query`에는 검색엔진에 사용할 '검색 최적화 질의'를 생성합니다. 아래 규칙을 반드시 따릅니다.
        ...
        -   **카드 정보 치환 규칙**: '내 카드', '나의 카드', '보유 카드' 등이 포함된 경우, 사용자 정보의 카드 데이터를 **모든 속성**을 포함하여 치환합니다.
            * **완전한 카드 정보 활용**: bank(은행명), name(카드명), type(카드타입), ipn(국제브랜드) 모든 정보를 검색어에 포함
            * **예시**: "내 카드 혜택" → "우리카드 VVIP카드 신용카드 VISA 혜택"
            * **다중 카드**: 여러 카드를 보유한 경우 모든 카드 정보를 포함
...
{{
    ...
    "data": {{
        ...
        {{
            ...
            "name": "VVIP 카드",      // 카드명
            "paymentDate": "4",       // 결제일
            "ipn": "VISA",           // 국제브랜드 (VISA, Master, JCB 등)
            "type": "신용카드"        // 카드 타입
        }}
        ]
    }}
...
- **사용자 정보**: {{
    ...
    "data": {{
      ...
      "ownCardArr": [
        {{ "bank": "우리카드", "name": "VVIP카드", "type": "신용카드", "ipn": "VISA", "paymentDate": "4" }}
      ]
    }}
  }}
...
- **사용자 정보**: {{
    ...
    "data": {{
      ...
      "ownCardArr": [
        {{ "bank": "BC카드", "name": "VVIP카드", "type": "신용카드", "ipn": "VISA", "paymentDate": "4" }}
      ]
    }}
  }}
- **대화 기록**: "사용자: 내 카드 혜택 알려줘 / 어시스턴트: 네, BC카드는..."
...
{{
  ...
  "refined_query": "사용자 카드 결제일자",
  "reasoning": "질문에 대한 답변이 제공된 사용자 정보 안에 모두 포함되어 있으므로 USER_INFO_ONLY로 분류했습니다. 별도의 외부 문서 검색이 필요 없습니다."
}}
