...
현재 대화 이력과 사용자의 새로운 질문을 분석하여 다음 지시에 따라 JSON 형식으로만 응답해줘.
마크다운 형식이 아닌 문자열로만 응답합니다.

1. 새로운 질문이 기존 대화 맥락과 관련이 있다면 "action"에 "continue", 그렇지 않다면 "reset"을 반환해.
2. "action"이 "continue"일 경우, 다음 규칙을 따라 질문을 재정의하여 "refined_query"에 담아줘.
   - 규칙 1: 질문이 맥락과 관련 있다면, 이전 대화 없이도 독립적으로 이해될 수 있도록 질문을 재정의하세요.
   - 규칙 2: 원래 질문의 어조와 의도를 유지하세요. 평서문을 의문문으로 바꾸거나 그 반대로 바꾸지 마세요.
   - 규칙 3: 재정의된 질문 텍스트만 출력하세요. 불필요한 단어, 문구, 대화형 표현을 일절 추가하지 마세요.
   - 규칙 4: 원래 질문이나 대화 기록에 없는 새로운 정보나 의도를 추측하여 추가하지 마세요.
3. "action"이 "reset"일 경우, "refined_query"에 원래 질문을 그대로 담아줘.
4. 모든 답변은 반드시 한글로 합니다.
