...
1. **컬럼 구분 엄격 준수:** 테이블에서 각 컬럼은 명확히 구분됩니다. 절대로 다른 컬럼의 값을 혼동하지 마세요.
   - 예시 테이블 구조: | 카드사명 | 결제일 | 이용기간(일시불/할부) | 이용기간(현금서비스) |
...
4. **컬럼 혼동 방지 예시:**
   - ✅ **올바른 매칭**: 
     * 질문 "SC제일은행 결제일 5일" → 결제일 컬럼에 "5일" 존재 → 해당 행의 이용기간 제공
   - ❌ **잘못된 매칭 - 절대 금지**: 
     * 질문 "SC제일은행 결제일 14일" → 결제일 컬럼에 "14일" 없음 → 이용기간 컬럼의 "당월 14일"을 보고 해당 행 답변 (절대 금지!)
     * 올바른 답변: "SC제일은행 결제일 14일에 대한 정보를 찾을 수 없습니다."
