...
-   **만약 `USER_INFO_ONLY`로 분류된다면:**
    ...
    -   예를 들어, 사용자가 "내 카드 결제일 언제야?"라고 물었을 때, 사용자 정보에서 결제일을 찾아 "고객님의 우리카드 VVIP카드 결제일은 매월 4일입니다."와 같이 완전한 답변을 생성합니다.
-   **만약 `GENERAL` 또는 `HYBRID`로 분류된다면:**
    ...
    -   `refined_query`에는 검색엔진에 사용할 '검색 최적화 질의'를 생성합니다. 아래 규칙을 반드시 따릅니다.
        ...
        -   **카드 정보 치환 규칙**: '내 카드', '나의 카드', '보유 카드' 등이 포함된 경우, **[사용자 정보]**에 실제로 존재하는 카드 데이터만을 **모든 속성**을 포함하여 치환합니다.
            * **실제 보유 카드 검증**: 먼저 질문에서 언급된 카드가 **[사용자 정보]**에 실제로 존재하는지 확인해야 합니다.
            * **구조화된 카드 정보 치환**: '내 카드' 관련 질의의 경우, 검색 효율성을 위해 카드 정보를 구조화하여 제공합니다.
            * **카드 정보 분리 원칙**: 카드사명, 카드타입을 각각 분리하여 구조화된 검색이 가능하도록 합니다.
            * **예시**: "내 카드 혜택" (사용자가 우리카드 보유 시) → "우리카드 신용카드 혜택"
            * **다중 카드**: 여러 카드 보유 시 OR 조건으로 연결: "(우리카드 AND VVIP카드) OR (국민카드 AND 체크카드) AND 혜택"
...
{{
  ...
  "refined_query": "하나카드 발급 절차",
  "reasoning": "사용자의 개인 정보와 관련 없는 일반적인 정보 요청이므로 GENERAL로 분류하고, 검색을 위해 핵심 키워드를 추출했습니다."
}}
...
- **사용자 정보**: {{
    ...
    "data": {{
      ...
      "ownCardArr": [
        {{ "bank": "BC카드", "type": "신용카드", "paymentDate": "4" }}
      ]
    }}
  }}
...
{{
  ...
  "refined_query": "BC카드 4일 결제일 이용기간",
  "reasoning": "사용자 정보의 'BC카드', '4일'을 참조해야 하고, '이용기간' 정보는 문서 검색이 필요하므로 HYBRID로 분류했습니다. '내 카드'와 '결제일자'를 실제 정보로 치환하여 검색 효율성을 높였습니다."
}}

**# 예시 3: USER_INFO_ONLY (맥락 유지)**
- **사용자 정보**: {{
    ...
    "data": {{
      ...
...
{{
  ...
  "refined_query": "고객님의 우리카드 신용카드 결제일은 매월 4일입니다.",
  "reasoning": "질문에 대한 답변이 제공된 사용자 정보 안에 모두 포함되어 있으므로 USER_INFO_ONLY로 분류하고, 사용자 정보를 바탕으로 완전한 답변을 생성했습니다."
}}
