...
-   **만약 `GENERAL` 또는 `HYBRID`로 분류된다면:**
    ...
    -   `refined_query`에는 검색엔진에 사용할 '검색 최적화 질의'를 생성합니다. 아래 규칙을 반드시 따릅니다.
        ...
        -   '새로운 질문'에 '내', '나', '저', '제가'와 같은 나를 지칭하는 표현이 포함된 경우, 질문의 의도에 따라 **이름을 제외한** 사용자 정보를 활용하여 재정의합니다.
        -   예를 들어, "내 카드" 등은 **[사용자 정보]**의 실제 값(예: "우리카드")으로, '내 나이'와 관련된 질문이면 **[사용자 정보]**의 실제 값(예: "30세")을 활용하는 방식입니다.
        -   사용자 '이름'은 답변에 필수적인 경우가 아니면 질의에 포함하지 않습니다.
        -   '대화 이력'의 맥락을 적극 활용하여 질문을 재정의합니다.
...
**# 예시 1: GENERAL (새로운 주제)**
- **사용자 정보**: { "card_info": { "card_name": "우리카드" } }
